...
if __name__ == "__main__":
    ...
    Note that we did not cover all of these techniques in class, so you might need to do some exploration on your own.
    """
    ...
    u = (4, 5, 2)
    print(t[1] + u[0])          #6
    print(t + u)                #(1, 2, 3, 4, 5, 2)
    print(s[1] * t[2])          #bbb
    print(hmmm(u))              #(4, 5 ,2)
    print(hmmm((5, 2, 3)))      #(2, 5)

    print("*" * 100)
    """
    Write a function called add_tuples that takes three tuples, 
    each with two values, and returns a single tuple with two values containing the sum of the values in the tuples. 
    ...
    def add_tuples(FirstTuple, SecondTuple, Thirdtuple):
        return (FirstTuple[0] + SecondTuple[0] + Thirdtuple[0], FirstTuple[1] + SecondTuple[1] + Thirdtuple[1])
